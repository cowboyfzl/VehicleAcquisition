...
//    //NSLog(@"\n********************************************************");
    ...
    else if (type == BAHttpRequestTypeJOSNPost || type == BAHttpRequestTypeJSONPut || type == BAHttpRequestTypeJSONPatch)
    {
        AFURLSessionManager *manager = [self sharedURLSession];
        NSString * method = @"";
        switch (type) {
            case BAHttpRequestTypeJOSNPost:
                method = @"POST";
                break;
            case BAHttpRequestTypeJSONPut:
                method = @"PUT";
                break;
            case BAHttpRequestTypeJSONPatch:
                method = @"PATCH";
                break;
            default:
                break;
...
//                        errorDic[ERRORKEY] = responseObject;
                        [error setValue:[errorDic copy] forKey:@"userInfo"];
                    }
                    failureBlock(error);
                }
            }
        ...
        [task resume];
    } else if (type == BAHttpRequestTypeJSONDelete) {
        BANetManagerShare.sessionManager.requestSerializer.HTTPMethodsEncodingParametersInURI = [NSSet setWithObjects:@"GET", @"HEAD", nil];
        //发送网络请求(请求方式为DELETE)
        [BANetManagerShare.sessionManager DELETE:URLString parameters:parameters success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            if (successBlock)
            {
                successBlock(responseObject);
            }
...
/**
 ...
 @return BAURLSessionTask
 */
+ (BAURLSessionTask *)ba_request_POSTJSONWithEntity:(BADataEntity *)entity
                                       successBlock:(BAResponseSuccessBlock)successBlock
                                       failureBlock:(BAResponseFailBlock)failureBlock
                                      progressBlock:(BADownloadProgressBlock)progressBlock
{
    ...
    return [self ba_requestWithType:BAHttpRequestTypeJOSNPost isNeedCache:entity.isNeedCache urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}

/**
 JSON包裹BODY网络请求的实例方法 Delete
 ...
 @return BAURLSessionTask
 */
+ (BAURLSessionTask *)ba_request_PUTJSONWithEntity:(BADataEntity *)entity
                                       successBlock:(BAResponseSuccessBlock)successBlock
                                       failureBlock:(BAResponseFailBlock)failureBlock
                                      progressBlock:(BADownloadProgressBlock)progressBlock
{
    ...
    return [self ba_requestWithType:BAHttpRequestTypeJSONPut isNeedCache:entity.isNeedCache urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}

/**
 JSON包裹BODY网络请求的实例方法 PATCH
 ...
 @return BAURLSessionTask
 */
+ (BAURLSessionTask *)ba_request_PATCHJSONWithEntity:(BADataEntity *)entity
                                        successBlock:(BAResponseSuccessBlock)successBlock
                                        failureBlock:(BAResponseFailBlock)failureBlock
                                       progressBlock:(BADownloadProgressBlock)progressBlock {
    if (!entity || ![entity isKindOfClass:[BADataEntity class]]) {
    ...
    return [self ba_requestWithType:BAHttpRequestTypeJSONPatch isNeedCache:entity.isNeedCache urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
...
/**
 ...
 @return BAURLSessionTask
 */
+ (BAURLSessionTask *)ba_request_DELETEJSONWithEntity:(BADataEntity *)entity
                                     successBlock:(BAResponseSuccessBlock)successBlock
                                     failureBlock:(BAResponseFailBlock)failureBlock
                                    progressBlock:(BADownloadProgressBlock)progressBlock
{
    ...
    return [self ba_requestWithType:BAHttpRequestTypeJSONDelete isNeedCache:NO urlString:entity.urlString parameters:entity.parameters successBlock:successBlock failureBlock:failureBlock progressBlock:progressBlock];
}
                                    ...
                                    progressBlock:(BAUploadProgressBlock)progressBlock
{
    ...
    sessionTask = [BANetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        NSError *error = nil;
        [formData appendPartWithFileURL:[NSURL URLWithString:fileEntity.filePath] name:@"file" fileName:fileEntity.fileName mimeType:fileEntity.mineType error:&error];
